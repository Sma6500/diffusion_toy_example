...
def ddim_statistics(
    ...
    """Compute DDIM coefficients as described in [2].
...
def plot_score_vector_field(
    ...
    """
    Plot score function with a hard to understand code (for nothing)
    basically it computes the score given by score_func on a grid defined by x_range, y_range
    and plot the corresponding gradient vector field.
    ...
    t : int
        timestep at which you want to evaluate your score function.
    """
    if ax is None:
        ax = plt
    if isinstance(t, int):
    ...
    U, V = U.detach().cpu().numpy(), V.detach().cpu().numpy()
